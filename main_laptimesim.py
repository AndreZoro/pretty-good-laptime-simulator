...
def main(
    ...
    parfilepath = os.path.join(
        repo_path, "laptimesim", "input", "tracks", "track_pars.ini"
    )
    ...
    # create instance
    driver = laptimesim.src.driver.Driver(
        carobj=car, pars_driver=driver_opts, trackobj=track, stepsize=track.stepsize
    )
    ...
    lap = laptimesim.src.lap.Lap(
        driverobj=driver, trackobj=track, pars_solver=solver_opts, debug_opts=debug_opts
    )
    ...
    if not sa_opts["use_sa"]:
        ...
        sa_range_1 = np.linspace(
            sa_opts["range_1"][0], sa_opts["range_1"][1], sa_opts["range_1"][2]
        )
    ...
    if not sa_opts["use_sa"] and debug_opts["use_print_result"]:
        ...
        print("-" * 50)
        print(
            "Consumption: %.2f kg/lap | %.2f kJ/lap"
            % (lap.fuel_cons_cl[-1], lap.e_cons_cl[-1] / 1000.0)
        )
        # [J] -> [kJ]
        print("-" * 50)
        ...
        print("Forward/Backward Plus Solver")
        print(
            "Runtime for sensitivity analysis: %.2f s" % (time.perf_counter() - t_start)
        )
        print("-" * 50)
        ...
        if sa_opts["sa_type"] == "mass":
            ...
            print(
                "Average sensitivity of lap time to mass: %.3f s/kg"
                % (t_lap_diff / m_diff)
            )
            print(
                "Average sensitivity of fuel consumption to mass: %.5f kg/kg"
                % (fuel_cons_diff / m_diff)
            )
            print("-" * 50)
    ...
    output_path = os.path.join(
        output_path_velprofile, "velprofile_" + track_opts["trackname"].lower() + ".csv"
    )
    ...
    result_objects_file_path = os.path.join(
        output_path_testobjects,
